"""Data loader for VWAP + HMM backtests on SPY, QQQ, IWM.

Fetching order (first success wins):
1. Local cached CSV in ./data/<TICKER>.csv
2. Yahoo Finance via yfinance (real data, requires internet)
3. Fallback: real S&P 500 index OHLCV (1950-2018) from vijinho/sp500 for SPY,
   plus factor-calibrated proxies for QQQ and IWM derived from it.

The fallback proxies for QQQ/IWM are statistical clones calibrated to published
historical characteristics vs. the S&P 500 — they are NOT real market prints.
Real data replaces them automatically when yfinance is reachable.
"""
from __future__ import annotations

import io
from pathlib import Path
from typing import Optional

import numpy as np
import pandas as pd

DATA_DIR = Path(__file__).resolve().parent.parent / "data"
DATA_DIR.mkdir(exist_ok=True)

TICKERS = ("SPY", "QQQ", "IWM")

SP500_FALLBACK_URL = (
    "https://raw.githubusercontent.com/vijinho/sp500/master/csv/sp500.csv"
)

# Beta / vol / drift calibration of proxies vs SPY (from historical Russell/NDX vs SPX):
#   QQQ: beta~1.15, excess vol ~30%, small positive alpha (tech-sector tilt).
#   IWM: beta~1.20, excess vol ~35%, small-cap premium small in recent decades.
PROXY_PARAMS = {
    "QQQ": dict(beta=1.15, idio_vol=0.006, alpha=0.00008, vol_mul=1.10, seed=17),
    "IWM": dict(beta=1.20, idio_vol=0.007, alpha=0.00002, vol_mul=1.15, seed=31),
}


def _yf_download(ticker: str, start: str, end: str) -> Optional[pd.DataFrame]:
    try:
        import logging, warnings
        logging.getLogger("yfinance").setLevel(logging.CRITICAL)
        warnings.filterwarnings("ignore")
        import yfinance as yf
        df = yf.download(ticker, start=start, end=end, interval="1d",
                         auto_adjust=False, progress=False)
        if df is None or len(df) == 0:
            return None
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)
        df.index = pd.to_datetime(df.index)
        need = {"Open", "High", "Low", "Close", "Volume"}
        if not need.issubset(df.columns):
            return None
        return df[list(need | {"Adj Close"} & set(df.columns))].dropna()
    except Exception:
        return None


def _load_sp500_fallback() -> pd.DataFrame:
    """Fetch the vijinho/sp500 daily OHLCV (1950-2018)."""
    import urllib.request
    req = urllib.request.Request(SP500_FALLBACK_URL,
                                 headers={"User-Agent": "Mozilla/5.0"})
    with urllib.request.urlopen(req, timeout=30) as r:
        raw = r.read().decode("utf-8")
    df = pd.read_csv(io.StringIO(raw))
    df["Date"] = pd.to_datetime(df["Date"])
    df = df.set_index("Date").sort_index()
    df = df[["Open", "High", "Low", "Close", "Adj Close", "Volume"]].astype(float)
    return df


def _synthesize_proxy(spy: pd.DataFrame, ticker: str) -> pd.DataFrame:
    """Build a statistically-realistic proxy OHLCV from SPY returns.

    Uses a single-factor model:   r_t = alpha + beta * r_spy_t + eps_t
    Intraday range and volume are scaled versions of SPY's, with noise.
    """
    p = PROXY_PARAMS[ticker]
    rng = np.random.default_rng(p["seed"])
    n = len(spy)
    r_spy = np.log(spy["Close"] / spy["Close"].shift(1)).fillna(0.0).values
    eps = rng.normal(0.0, p["idio_vol"], size=n)
    r = p["alpha"] + p["beta"] * r_spy + eps

    # price path starting at a sensible level
    start_price = 40.0 if ticker == "QQQ" else 45.0
    close = start_price * np.exp(np.cumsum(r))

    # intraday range scaled from SPY's range
    spy_range_pct = (spy["High"] - spy["Low"]) / spy["Close"]
    range_pct = (spy_range_pct.values * p["vol_mul"]
                 + rng.normal(0.0, 0.001, size=n)).clip(0.002, None)

    # open = previous close * gap, gap ~ small noise around r_t partial
    gap = rng.normal(0.0, p["idio_vol"] * 0.5, size=n)
    open_ = np.empty(n)
    open_[0] = close[0] * (1 - r[0]) if n else start_price
    open_[1:] = close[:-1] * np.exp(gap[1:])
    hi = np.maximum(open_, close) * (1 + range_pct * 0.5)
    lo = np.minimum(open_, close) * (1 - range_pct * 0.5)

    # volume: scale SPY volume by a factor related to relative market cap / ADV
    vol_scale = 0.9 if ticker == "QQQ" else 0.55
    noise = rng.lognormal(0.0, 0.15, size=n)
    volume = (spy["Volume"].values * vol_scale * noise).astype(np.int64)

    out = pd.DataFrame(
        {
            "Open": open_,
            "High": hi,
            "Low": lo,
            "Close": close,
            "Adj Close": close,
            "Volume": volume,
        },
        index=spy.index,
    )
    return out


def load_daily(ticker: str, start: str = "2007-01-01", end: str = "2024-12-31",
               refresh: bool = False) -> pd.DataFrame:
    """Load daily OHLCV for ticker. Columns: Open, High, Low, Close, Adj Close, Volume."""
    cache = DATA_DIR / f"{ticker}.csv"
    if cache.exists() and not refresh:
        df = pd.read_csv(cache, index_col=0, parse_dates=True)
        return df.loc[start:end].dropna()

    # 1. try real data via yfinance
    df = _yf_download(ticker, start, end)
    if df is not None and len(df) > 200:
        df.to_csv(cache)
        return df.loc[start:end].dropna()

    # 2. fallback: S&P 500 index + proxies
    spy = _load_sp500_fallback()
    (DATA_DIR / "_SPY_SOURCE.csv").write_text(
        "Real S&P 500 index OHLCV from github.com/vijinho/sp500 (1950-2018).\n"
    )
    spy.to_csv(DATA_DIR / "SPY.csv")
    if ticker == "SPY":
        return spy.loc[start:end].dropna()
    # synthesize QQQ/IWM proxies aligned to SPY dates
    proxy = _synthesize_proxy(spy, ticker)
    proxy.to_csv(cache)
    return proxy.loc[start:end].dropna()


def load_all(start: str = "2007-01-01", end: str = "2024-12-31",
             refresh: bool = False) -> dict[str, pd.DataFrame]:
    return {t: load_daily(t, start, end, refresh) for t in TICKERS}


if __name__ == "__main__":
    for t in TICKERS:
        d = load_daily(t)
        print(f"{t}: {len(d)} rows  {d.index.min().date()} -> {d.index.max().date()}"
              f"  close[last]={d['Close'].iloc[-1]:.2f}")
